...
export default class ContextVariableListView extends UiComponent {
  ...
  render () {
    const {context,state,variableContext} = this.props;
    ...
    const variables = variableContext.variables.map((variable,index) => {
      return <ContextVariableView key={variable.identifier + ":" + index} context={context} variable={variable} parent={path} />
    });
    ...
    return <li className="context-variable-list-view">
      <details data-name="dataname">
        <summary class="php-debug-variable-title">
          {labels}
        </summary>
        <ul>
        {variables}
